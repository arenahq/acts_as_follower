  ...
  module Follower
    ...
    module InstanceMethods
      ...
      def following?(followable)
        Follow.unblocked.for_follower(self).for_followable(followable).count.positive?
      end
      ...
      def all_following(options = {})
        all_follows(options).collect(&:followable)
      end
      ...
      def method_missing(m, *args)
        if m.to_s[/following_(.+)_count/]
          following_by_type_count(Regexp.last_match(1).singularize.classify)
        elsif m.to_s[/following_(.+)/]
          following_by_type(Regexp.last_match(1).singularize.classify)
        else
          super
        end
      end
