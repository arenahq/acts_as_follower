module ActsAsFollower
  module FollowerLib
    ...
    def apply_options_to_scope(scope, options = {})
      ...
      scope = scope.includes(options[:includes]) if options.has_key?(:includes)
      scope = scope.joins(options[:joins]) if options.has_key?(:joins)
      scope = scope.where(options[:where]) if options.has_key?(:where)
      scope = scope.order(options[:order]) if options.has_key?(:order)
